...
def readVideo():
    ...
    inpImage = cv2.VideoCapture("input/test.mp4")
...
def perspectiveWarp(inpImage):
    ...
    src = np.float32([[700, 450],   # upperLEFT
                      [800, 450],   # upperRIGHT
                      [275, 720],   # lowerLEFT
                      [1200, 720]]) # lowerRIGHT

    # Window to be shown
    dst = np.float32([[500, 400],  # upperLEFT
                      [900, 400],  # upperRIGHT
                      [500, 720],  # lowerLEFT
                      [900, 720]]) # lowerRIGHT
...
def measure_lane_curvature(ploty, leftx, rightx):
    ...
    if leftx[0] - leftx[-1] > 60:
        curve_direction = 'To the Left'
    elif leftx[-1] - leftx[0] > 60:
        curve_direction = 'To the Right'
    else:
...
def offCenter(meanPts, inpFrame):
    ...
    direction = "To the Left" if deviation < 0 else "To the Right"
...
def addText(img, radius, direction, deviation, devDirection):
    ...
    if (direction != 'Straight'):
        text = 'Curvature Degree of the Road: ' + '{:04.0f}'.format(radius) + 'm'
        text1 = 'Curvature Direction: ' + (direction)

    else:
        text = 'Curvature Degree of the Road: ' + 'N/A'
        text1 = 'Curvature Degree of the Road: ' + (direction)

    cv2.putText(img, text, (50, 100), font, 0.7, (50, 150, 250), 2, cv2.LINE_AA)
    cv2.putText(img, text1, (50, 150), font, 0.7, (50, 150, 250), 2, cv2.LINE_AA)

    # Deviation
    deviation_text = 'Out of Center: ' + \
        str(round(abs(deviation), 3)) + ' m ' + devDirection
    cv2.putText(img, deviation_text, (50, 200), cv2.FONT_HERSHEY_TRIPLEX,
                0.7, (50, 150, 250), 2, cv2.LINE_AA)
...
image = cv2.VideoCapture('input/test.mp4')
video_src = 'test.mp4'
...
while (image.isOpened()):
    ...
    # Car cascade CascadeClassifier
    #grii = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    #cars = car_cascade.detectMultiScale(grii, 1.1, 1)
    #
    #for (x, y, w, h) in cars:
